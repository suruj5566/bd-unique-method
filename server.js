...
// ================================================================
//  FAST X OTP API (আপনার সক্রিয় API Key)
// ================================================================
...
app.post('/api/fastx/get-number', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('FASTX Response not JSON:', text);
      return res.json({ success: false, message: 'API থেকে সঠিক রেসপন্স আসছে না', raw: text });
    }
    ...
    if (data.ok !== false && data.data) {
      ...
      res.json({ success: false, message: data?.message || 'নাম্বার পাওয়া যায়নি', raw: data });
    }
  } catch (error) {
    console.error('FASTX Error:', error);
...
app.get('/api/fastx/check-otp', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('FASTX Check Response not JSON:', text);
      return res.json({ success: false, message: 'API থেকে সঠিক রেসপন্স আসছে না', raw: text });
    }
    ...
    if (data.ok && data.data) {
      ...
      res.json({ success: false, message: data?.message || 'কোনো তথ্য পাওয়া যায়নি', raw: data });
    }
  } catch (error) {
    console.error('FASTX Check Error:', error);
    res.json({ success: false, message: error.message });
  }
});

// 3. ব্যালেন্স চেক
app.get('/api/fastx/balance', async (req, res) => {
  try {
    const response = await fetch(`${FASTX_BASE}/user/balance`, {
      method: 'GET',
      headers: {
        'X-API-KEY': FASTX_API_KEY,
        'Accept': 'application/json'
  ...
  otpCheckInterval = setInterval(async () => {
    try {
      ...
      for (const entry of pendingNumbers) {
        try {
          const response = await fetch(`${FASTX_BASE}/user/checknum?nomor=${encodeURIComponent(entry.number)}`, {
          ...
          try {
            ...
            console.error('OTP Checker JSON Error:', text);
            continue;
          }
...
/fastx-get — FAST X OTP থেকে নাম্বার নিন
/fastx-check [নাম্বার] — OTP চেক করুন
/help — সাহায্য দেখুন`);
    } 
    else if (text === '/help') {
...
/fastx-get — FAST X OTP থেকে নতুন নাম্বার নিন
/fastx-check [নাম্বার] — OTP চেক করুন
...
🆔 আইডি: ${available.id}`);
    ...
    //  FAST X OTP কমান্ড
    // ============================================================
    else if (text === '/fastx-get') {
      try {
        const response = await fetch(`https://bd-unique-method.vercel.app/api/fastx/get-number`, { method: 'POST' });
        const data = await response.json();
        if (data.success) {
          const num = data.number?.number || data.number?.copy || 'N/A';
          await sendTelegramMessage(chatId, `✅ **FAST X OTP নাম্বার প্রোভিশন হয়েছে!**
          
📞 নাম্বার: ${num}
🌍 দেশ: ${data.number?.country || 'Unknown'}`);
        } else {
          await sendTelegramMessage(chatId, `❌ ${data.message || 'API Error'}`);
        }
      } catch (error) {
        await sendTelegramMessage(chatId, `❌ Error: ${error.message}`);
      }
    }
    else if (text.startsWith('/fastx-check ')) {
      const number = text.split(' ')[1];
      if (!number) {
        await sendTelegramMessage(chatId, '❌ নাম্বার দিন: /fastx-check 447384512345');
        return;
      }
      try {
        const response = await fetch(`https://bd-unique-method.vercel.app/api/fastx/check-otp?number=${number}`);
        const data = await response.json();
